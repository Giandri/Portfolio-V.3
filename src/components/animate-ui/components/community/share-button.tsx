...
import { Share2, Github, Instagram, Linkedin } from 'lucide-react';
...
const buttonVariants = cva(
  "relative overflow-hidden cursor-pointer inline-flex items-center justify-center whitespace-nowrap rounded-lg text-[10px] font-mono transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      size: {
...
type ShareButtonProps = HTMLMotionProps<'button'> & {
  ...
  className?: string;
  onIconClick?: (platform: 'github' | 'instagram' | 'linkedin') => void;
...
function ShareButton({
  ...
  return (
    <motion.button
      className={cn(
        'bg-primary text-sm font-mono text-primary-foreground hover:bg-primary/90',
