...
import { createFileTreeIconResolver, getBuiltInSpriteSheet, prepareFileTreeInput, type FileTreeSortComparator, type FileTreeSortEntry } from "@pierre/trees";
...
function FileSystemDocumentPreviewFallback({
  ...
  return (
    <div className={cn("flex h-full min-h-0 w-full flex-col items-center justify-center gap-2 rounded-2xl bg-muted/30 p-8 text-center", className)}>
      <span className="text-sm font-medium text-foreground">{fileName ?? "Document"}</span>
      <span className="max-w-xs text-xs text-muted-foreground">Inline preview is available with the @extend document viewer packages.</span>
    </div>
  );
}
...
function fileTypeFilterGroup(mime: string): FileTypeFilterGroup {
  ...
  if (mime === "application/vnd.ms-excel" || mime === "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" || mime === "text/csv" || mime === "text/tab-separated-values") {
    return "Spreadsheets";
  }
...
function FileSystemViewerLoading() {
  return (
    <div className="grid h-full min-h-48 flex-1 place-items-center bg-white/10 dark:bg-black/20">
...
const FILE_TYPE_FILTER_GROUPS: FileTypeFilterGroup[] = ["Documents", "Spreadsheets", "Images", "Code", "Text", "Archives & binary"];
...
const DATE_FILTER_PRESETS = ["1 day ago", "3 days ago", "1 week ago", "1 month ago", "3 months ago", "6 months ago", "1 year ago"];
...
function FileSystemFolderGlyph({ name, className }: { name?: string; className?: string }) {
  if (name?.toLowerCase() === "trash") {
    // eslint-disable-next-line @next/next/no-img-element -- The trash bin is a raster dock icon, not a sprite glyph.
    return <img src="/images/dock/bin.png" alt="" aria-hidden="true" draggable={false} className={className} />;
  }
...
function FileGenericPreview({ file }: { file: FileEntry }) {
  ...
  return (
    <div data-file-system-on-light="" className="flex size-full flex-col items-center justify-center gap-1.5 bg-white text-neutral-400 dark:bg-neutral-100">
...
function useVirtualWindow({
  ...
  React.useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      setWindow((previous) => {
        if (previous.end <= count && previous.start <= Math.max(0, firstVisible - 1) && previous.end >= Math.min(count, lastVisible + 1)) {
          return previous;
        }
        return {
...
function scrollIndexIntoView({ horizontal = false, index, itemSize, itemStride, leadingPx = 0, viewport }: { horizontal?: boolean; index: number; itemSize: number; itemStride: number; leadingPx?: number; viewport: HTMLDivElement | null }) {
...
function FileVisual({
  ...
  const previewUrl = previewUrls[clampedPageIndex] ?? lazyPageUrls[clampedPageIndex] ?? pageUrlCache?.get(`${file.path}#${clampedPageIndex}`) ?? null;
  ...
  const thumbnail = (
    <FileThumbnail
      ...
      previewContent={previewUrl || isLazyPagePending ? undefined : (customPreview ?? <FileGenericPreview file={file} />)}
    />
  );
...
export function FileSystem({
  ...
  const toggleSortColumn = React.useCallback((key: FileSystemSortKey) => {
    setSort((previous) => (previous.key === key ? { direction: previous.direction === "asc" ? "desc" : "asc", key } : { direction: defaultSortDirection(key), key }));
  }, []);
  ...
  const setDatePresetFilter = React.useCallback((type: FileSystemDateFilterType, preset: string) => {
    ...
    setFilters((previous) => [...previous.filter((filter) => filter.type !== type), { id, operator: "after", type, value: [preset] }]);
  }, []);
  ...
  const attachedStagePaths = React.useMemo(() => {
    ...
    const attached = [...stagePool].sort((a, b) => (stageRecency.get(b) ?? 0) - (stageRecency.get(a) ?? 0)).slice(0, GALLERY_STAGE_ATTACHED_COUNT);
  ...
  return (
    <div
      ...
      className={cn("flex h-[480px] min-h-0 flex-col overflow-hidden rounded-xl border bg-background text-foreground outline-none", className)}>
      ...
      <div className="relative grid h-12 shrink-0 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2 border-b bg-muted/40 px-2">
        ...
        : <Tabs value={view} onValueChange={(value) => setView(value as FileSystemView)} className="gap-0">
            <TabsList className="h-8 p-0.5">
              {VIEW_OPTIONS.map((option) => (
                <TabsTrigger key={option.value} value={option.value} aria-label={`${option.label} view`} title={option.label} className="h-7 grow-0 px-2.5 sm:h-7">
                  <HugeiconsIcon icon={option.icon} className="size-4" />
                </TabsTrigger>
              ))}
            </TabsList>
        ...
        <div className="flex min-w-0 items-center justify-end gap-1">
          <FileSystemSortSelect layout={headerLayout} onKeyChange={applySortKey} showLabel={!isBelowIpadWidth} sort={sort} />
          <FileSystemFilterMenu fileTypeOptions={fileTypeOptions} filters={filters} onOpenCustomRange={openDateRangeDialog} onSelectDatePreset={setDatePresetFilter} onToggleFileType={toggleFileTypeFilterValue} />
          <FileSystemSearchField inputRef={searchInputRef} isExpanded={isSearchExpanded} layout={headerLayout} onExpandedChange={setIsSearchExpanded} onValueChange={setSearchInput} value={searchInput} />
        </div>
      </div>
      {hasActiveFilters ?
        <div className="flex shrink-0 flex-wrap items-center gap-1 border-b bg-muted/20 px-2 py-1.5 text-xs text-muted-foreground">
          {filters.map((filter) => {
            ...
            return (
              <FileSystemFilterPill
                ...
                onOperatorChange={(operator) => setFilters((previous) => previous.map((entry) => (entry.id === filter.id ? { ...entry, operator } : entry)))}
                  ...
                  setFilters((previous) =>
                    previous.map((entry) =>
                      entry.id === filter.id ?
                        {
                          ...entry,
                          operator: entry.operator === "before" || entry.operator === "after" ? entry.operator : "after",
                          value: [preset],
                        }
                      : entry,
                    ),
          ...
          <button type="button" onClick={() => setFilters([])} className="rounded-md px-1.5 py-0.5 transition-colors outline-none hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring">
            Clear
          </button>
        </div>
      : null}
      <div className="relative min-h-0 flex-1">
        ...
        : currentEntries.length === 0 && (view !== "columns" || isSearching || hasActiveFilters) ?
          <FileSystemEmptyState
            label={
              isSearching ? `No results for “${searchInput.trim()}”`
              : hasActiveFilters ?
      ...
      <div aria-live="polite" className="flex h-7 shrink-0 items-center justify-center gap-1 border-t bg-muted/40 px-3 text-xs text-muted-foreground">
        <span>
      ...
      <Dialog
        ...
        {openedFile ?
          <DialogContent className={cn("overflow-hidden p-0", VIEWER_DIALOG_CLASSNAMES[openedFile.kind])} showCloseButton={openedFile.kind === "image"}>
            <DialogTitle className="sr-only">{openedFileName}</DialogTitle>
            {
              openedFile.kind === "image" ?
                ...
                <img src={openedFile.url} alt={openedFileName} className="max-h-[88vh] w-auto max-w-full rounded-2xl object-contain" />
...
const TOOLBAR_ICON_BUTTON_CLASSNAME = "flex size-7 shrink-0 items-center justify-center rounded-md text-muted-foreground transition-colors outline-none hover:bg-accent hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring";
...
function FileSystemSearchField({
  ...
  const input = (
    <div
      ...
      <HugeiconsIcon icon={Search01Icon} className="pointer-events-none absolute left-2 size-3.5 text-muted-foreground" />
      <input
        ref={inputRef}
        type="text"
        role="searchbox"
  ...
  return (
    <Popover open={isExpanded} onOpenChange={onExpandedChange}>
      <PopoverTrigger render={<button type="button" aria-label="Search" title="Search" className={cn(TOOLBAR_ICON_BUTTON_CLASSNAME, "relative")} />}>
...
function FileSystemSortSelect({ layout, onKeyChange, showLabel, sort }: { layout: "full" | "compact" | "minimal"; onKeyChange: (key: FileSystemSortKey) => void; showLabel: boolean; sort: FileSystemSortState }) {
  ...
  return (
    <Select value={sort.key} onValueChange={(value) => onKeyChange(value as FileSystemSortKey)}>
      <SelectTrigger size="sm" aria-label="Sort by" title="Sort by" className="h-7 min-h-7 w-auto min-w-0 shrink-0 [&_svg]:size-4">
        <SelectValue>
...
function FileSystemFileTypeCommand({ checkedMimes, onToggle, options }: { checkedMimes: string[]; onToggle: (mime: string, checked: boolean) => void; options: FileTypeFilterOption[] }) {
            ...
            return (
              <CommandGroup key={group} heading={group}>
                {groupOptions.map((option) => {
                  ...
                  return (
                    <CommandItem key={option.mime} value={option.label} keywords={[option.mime]} onSelect={() => onToggle(option.mime, !isChecked)}>
                      <HugeiconsIcon icon={Tick02Icon} className={cn("size-4 text-foreground", !isChecked && "opacity-0")} />
...
function FileSystemFilterMenu({
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger render={<Button type="button" variant="outline" size="icon-sm" aria-label="Filter" title="Filter" className="relative size-7 sm:size-7" />}>
      ...
      <DropdownMenuContent align="end" className="min-w-44">
        <DropdownMenuSub>
          ...
          <DropdownMenuSubContent className="w-60">
            <FileSystemFileTypeCommand checkedMimes={fileTypeFilter?.value ?? []} onToggle={onToggleFileType} options={fileTypeOptions} />
...
const FILTER_PILL_SEGMENT_CLASSNAME = "flex h-5 items-center gap-1 border border-l-0 bg-background px-1.5 whitespace-nowrap text-foreground";

const FILTER_PILL_BUTTON_CLASSNAME = cn(FILTER_PILL_SEGMENT_CLASSNAME, "transition-colors outline-none hover:bg-accent focus-visible:ring-2 focus-visible:ring-ring");
...
function FileSystemFilterPill({
  ...
  const selectedTypeLabels = filter.type === "fileType" ? filter.value.map((mime) => fileTypeOptions.find((option) => option.mime === mime)?.label ?? mime) : [];
  ...
  return (
    <div className="flex items-center text-xs">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger render={<button type="button" className={cn(FILTER_PILL_BUTTON_CLASSNAME, "text-primary")} />}>{FILTER_OPERATOR_LABELS[filter.operator]}</DropdownMenuTrigger>
      ...
      {filter.type === "fileType" ?
        <DropdownMenu>
          <DropdownMenuTrigger render={<button type="button" title={selectedTypeLabels.join(", ")} className={FILTER_PILL_BUTTON_CLASSNAME} />}>
            {filter.value.length === 1 ? selectedTypeLabels[0] : `${filter.value.length} selected`}
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start" className="w-60">
            <FileSystemFileTypeCommand checkedMimes={filter.value} onToggle={onToggleFileType} options={fileTypeOptions} />
          </DropdownMenuContent>
        </DropdownMenu>
      : isCustomRange ?
      ...
      : <DropdownMenu>
          <DropdownMenuTrigger render={<button type="button" className={FILTER_PILL_BUTTON_CLASSNAME} />}>{filter.value[0]}</DropdownMenuTrigger>
      ...
      <button type="button" aria-label={`Remove ${FILTER_TYPE_LABELS[filter.type]} filter`} onClick={onRemove} className={cn(FILTER_PILL_BUTTON_CLASSNAME, "rounded-r-md px-1 text-muted-foreground hover:text-foreground")}>
        <HugeiconsIcon icon={Cancel01Icon} className="size-3" />
      </button>
    </div>
  );
...
const DATE_RANGE_DIALOG_PRESETS = ["Last 7 days", "This month", "Last 1 month", "Last 3 months", "This year", "Last 12 months"];
...
function FileSystemRangeCalendar({ onSelect, range }: { onSelect: (range: { from?: Date; to?: Date }) => void; range: { from?: Date; to?: Date } }) {
    ...
    <div className="relative">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {months.map((month, monthIndex) => {
          ...
          const cells = [...Array.from({ length: firstWeekday }, () => null), ...Array.from({ length: dayCount }, (_, index) => new Date(month.getFullYear(), month.getMonth(), index + 1))];
...
function FileSystemDateRangeDialog({ initialRange, onApply, onClose }: { initialRange?: { from: Date; to: Date }; onApply: (from: Date, to: Date) => void; onClose: () => void }) {
  ...
  const dateField = (label: string, value: string, onChange: (value: string) => void) => (
    <div className="flex flex-1 flex-col gap-1.5">
      ...
      <div className="relative flex items-center">
        <HugeiconsIcon icon={Calendar03Icon} className="pointer-events-none absolute left-2.5 size-3.5 text-muted-foreground" />
        <Input type="text" value={value} placeholder="YYYY-MM-DD" aria-label={`${label} date`} onChange={(event) => onChange(event.target.value)} className="h-8 pl-8 sm:h-8" />
      </div>
    </div>
  );
        ...
        <DialogPanel className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {DATE_RANGE_DIALOG_PRESETS.map((preset) => (
              <Button key={preset} type="button" variant="outline" size="sm" onClick={() => selectRange(dateRangePresetRange(preset))}>
                {preset}
              </Button>
            ))}
          </div>
...
function useResolvedFileUrl(file: FileEntry | null, getFileUrl?: (file: FileSystemFileItem) => string | Promise<string>, cache?: Map<string, string>) {
...
function FileSystemEmptyState({ label, isLoading = false }: { label: string; isLoading?: boolean }) {
  return <div className={cn("flex size-full items-center justify-center text-sm text-muted-foreground", isLoading && "animate-pulse motion-reduce:animate-none")}>{label}</div>;
}
...
function isTypeAheadKey(event: React.KeyboardEvent) {
  return event.key.length === 1 && /^[\p{L}\p{N}]$/u.test(event.key) && !event.altKey && !event.ctrlKey && !event.metaKey;
}
...
function FileSystemIconsView({ entries, onOpen, onSelect, renderFilePreview, selectedPath }: FileSystemViewProps) {
  ...
  const tabStopPath = visibleEntries.some((entry) => entry.path === selectedPath) ? selectedPath : (visibleEntries[0]?.path ?? null);
          ...
          {visibleEntries.map((entry) => {
            ...
            return (
              <button
                ...
                <span className={cn("flex h-16 w-20 shrink-0 items-center justify-center rounded-lg p-1 transition-colors group-focus-visible:ring-2 group-focus-visible:ring-ring", isSelected && "bg-accent")}>
                  {entry.kind === "folder" ?
                    <FileSystemFolderGlyph name={entry.name} className="h-13 w-auto drop-shadow-sm" />
                  : <FileVisual
                      file={entry}
                      className={cn(
                ...
                <span className={cn("max-w-full rounded-sm px-1.5 py-px text-center text-xs leading-tight break-words", isSelected ? "bg-primary text-primary-foreground" : "text-foreground")}>
...
function FileSystemListColumnHeader({ className, label, onClick, sort, sortKey }: { className?: string; label: string; onClick: (key: FileSystemSortKey) => void; sort: FileSystemSortState; sortKey: FileSystemSortKey }) {
  ...
  return (
    <button
      ...
      className={cn("flex items-center gap-0.5 rounded-sm py-0.5 transition-colors outline-none hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring", isActive && "text-foreground", className)}>
      {label}
      {isActive ?
...
function FileSystemListView({ currentPath, fileFilter, index, onOpen, onSelect, onSortColumnClick, searchQuery, selectedPath, sort, treeExpansionRef }: FileSystemViewProps) {
  ...
  return (
    <div className="flex size-full flex-col">
      ...
      <div className="flex shrink-0 items-center border-b py-1 pr-6 pl-[46px] text-xs font-medium text-muted-foreground">
        <FileSystemListColumnHeader className="flex-1 justify-start" label="Name" onClick={onSortColumnClick} sort={sort} sortKey="name" />
        <FileSystemListColumnHeader className="w-44 justify-start" label="Date Modified" onClick={onSortColumnClick} sort={sort} sortKey="updatedAt" />
        <FileSystemListColumnHeader className="w-20 justify-start" label="Size" onClick={onSortColumnClick} sort={sort} sortKey="size" />
      </div>
      ...
      <FileSystemPierreTree
        ...
        index={index}
        initialSelectedPath={selectedPath?.startsWith(currentPath) ? selectedPath.slice(currentPath.length).replace(/\/$/, "") : null}
        onOpen={onOpen}
        onSelect={onSelect}
...
function FileSystemPierreTree({
  ...
  const preparedInput = React.useMemo(() => prepareFileTreeInput(relativePaths, { sort: sortComparator }), [relativePaths, sortComparator]);
  ...
  const { model } = useFileTree({
    ...
    renderRowDecoration: ({ row }) => {
      const entry = row.kind === "file" ? index.files.get(`${currentPath}${row.path}`) : index.folders.get(normalizeFolderPath(`${currentPath}${row.path}`));
  ...
  React.useLayoutEffect(() => {
    ...
    return () => {
      expansionStore.set(currentPath, hasActiveFiltersRef.current ? (preFilterExpansionRef.current ?? []) : collectExpandedDirectories(appliedPreparedInputRef.current.paths));
    };
  ...
  const resolveTreeItem = (relativePath: string) => model.getItem(relativePath) ?? model.getItem(relativePath.endsWith("/") ? relativePath.slice(0, -1) : `${relativePath}/`);
  ...
  return (
    <PierreFileTree
      ...
      onKeyDown={(event) => {
        ...
        const focusedIndex = visibleEntries.findIndex((entry) => entry.path.slice(currentPath.length).replace(/\/$/, "") === focusedPath);
...
function FileSystemColumnsView(props: FileSystemViewProps) {
  const { currentPath, index, loadPreviewImageUrl, loadingFolders, onOpen, onSelect, pageUrlCache, renderFilePreview, selectedEntry, selectedPath } = props;
  ...
  const handleKeyDown = (event: React.KeyboardEvent) => {
    if (!ARROW_KEYS.has(event.key)) {
      ...
      const siblings = selectedEntry && selectedPath?.startsWith(currentPath) ? (index.children.get(selectedEntry.parentPath) ?? []) : (index.children.get(currentPath) ?? []);
      const match = typeAhead(
        event,
        siblings,
  ...
  const columnPaths = React.useMemo(() => {
    ...
    const targetFolder = deferredSelectedEntry?.kind === "folder" ? deferredSelectedEntry.path : (deferredSelectedEntry?.parentPath ?? currentPath);
    ...
    <ScrollArea orientation="horizontal" viewportRef={scrollContainerRef} viewportClassName="overscroll-x-contain">
      ...
      <ScrollAreaPrimitive.Content className="flex h-full w-max" style={{ minWidth: "100%" }} onKeyDown={handleKeyDown}>
        ...
        {selectedFile ?
          ...
          <ScrollArea orientation="vertical" className="min-w-60 flex-1 contain-inline-size" viewportClassName="flex justify-center p-4">
            <div className="flex w-full max-w-lg flex-col items-stretch gap-3">
              ...
              <div
                ...
                <FileVisual file={selectedFile} className="w-full" loadPreviewImageUrl={loadPreviewImageUrl} pageable pageUrlCache={pageUrlCache} previewAspectRatio={0.78} renderFilePreview={renderFilePreview} />
              </div>
...
const FileSystemColumn = React.memo(function FileSystemColumn({
  ...
  return (
    <ScrollArea orientation="vertical" className="w-60 shrink-0 border-r" viewportRef={viewportRef} viewportClassName="p-1.5" viewportProps={{ "aria-label": "Files", role: "listbox" }}>
      {isLoading && entries.length === 0 ?
        <div className="animate-pulse px-2 py-1.5 text-xs text-muted-foreground motion-reduce:animate-none">Loading…</div>
      : <div
          className="relative"
          style={{
            ...
            {entries.slice(start, end).map((entry) => {
              ...
              return (
                <button
                  ...
                  {entry.kind === "folder" ?
                    <FileSystemFolderGlyph name={entry.name} className="h-3.5 w-auto shrink-0" />
                  : coverUrl ?
                    // eslint-disable-next-line @next/next/no-img-element -- Cover thumbnails come from caller-provided file preview URLs.
                    <img src={coverUrl} alt="" draggable={false} className="size-4 shrink-0 rounded-[3px] bg-white object-cover" />
                  : <FileTypeIcon fileName={entry.name} className="size-4 shrink-0" />}
                  <span className="min-w-0 flex-1 truncate">{entry.name}</span>
                  {entry.kind === "folder" && folderHasChildren(index, entry) ?
                    <HugeiconsIcon icon={ArrowRight01Icon} className={cn("size-3.5 shrink-0", !isSelected && "text-muted-foreground/60")} />
                  : null}
                </button>
              );
            })}
...
function FileSystemGalleryStage({
    ...
    return (
      <div className={viewerFrameClassName}>
        <React.Suspense fallback={<FileSystemViewerLoading />}>
          <LazyPDFViewer src={url} className={cn("h-full", isDialog && "min-h-0 overflow-hidden rounded-2xl")} fileName={file.name} showToolbar={isDialog} showUpload={false} toolbarActions={toolbarActions} />
        </React.Suspense>
      </div>
    );
  }
  ...
  return <FileVisual file={file} className="w-56 max-w-full" loadPreviewImageUrl={loadPreviewImageUrl} pageable pageUrlCache={pageUrlCache} previewAspectRatio={0.78} renderFilePreview={renderFilePreview} />;
}

function FileSystemGalleryView(props: FileSystemViewProps) {
  const { attachedStagePaths, entries, index, onOpen, onSelect, poolStagePath, registerStageHost, renderFilePreview, selectedEntry, selectedPath } = props;
  ...
  const typeAhead = useEntryTypeAhead();
  const activeEntry = selectedEntry && entries.some((entry) => entry.path === selectedEntry.path) ? selectedEntry : (entries[0] ?? null);
  ...
  // unrelated renders.
  const stageHostRefs = React.useMemo(() => new Map(attachedStagePaths.map((path) => [path, (element: HTMLElement | null) => registerStageHost(path, element)] as const)), [attachedStagePaths, registerStageHost]);
  ...
  return (
    <div className="flex size-full flex-col" onKeyDown={handleKeyDown}>
      ...
      <ScrollArea orientation="horizontal" className="order-last h-auto w-full shrink-0 border-t" viewportRef={stripViewportRef} viewportClassName="p-2">
        <div
          className="relative h-14 min-w-full"
          style={{
            width: entries.length ? entries.length * GALLERY_TILE_STRIDE - GALLERY_TILE_GAP : undefined,
          }}>
          <div role="listbox" aria-label="Files" className="absolute inset-y-0 flex items-center gap-1.5" style={{ left: stripStart * GALLERY_TILE_STRIDE }}>
            {entries.slice(stripStart, stripEnd).map((entry) => {
              ...
              return (
                <button
                  ...
                  title={entry.name}
                  className={cn("flex size-14 shrink-0 items-center justify-center rounded-md border border-transparent p-1 outline-none focus-visible:ring-2 focus-visible:ring-ring", isActive && "border-ring/40 bg-accent")}>
                  {entry.kind === "folder" ?
                    <FileSystemFolderGlyph name={entry.name} className="h-9 w-auto" />
                  : <FileVisual file={entry} className="w-9 rounded-sm" previewAspectRatio={0.78} renderFilePreview={renderFilePreview} />}
                </button>
              );
            })}
          </div>
      ...
      <div className="flex min-h-0 flex-1">
        <div className="relative flex min-h-0 min-w-0 flex-1 items-center justify-center p-3">
          {activeEntry?.kind === "folder" ?
            <FileSystemFolderGlyph name={activeEntry.name} className="h-40 max-h-full w-auto drop-shadow-md" />
          ...
          {attachedStagePaths.map((path) => {
            ...
            return <div key={path} ref={stageHostRefs.get(path)} inert={!isActiveStage || undefined} className={cn("absolute inset-0 flex items-center justify-center p-3", !isActiveStage && "invisible opacity-0")} />;
          })}
        </div>
        {activeEntry ?
          <ScrollArea orientation="vertical" className="hidden w-64 shrink-0 border-l sm:block" viewportClassName="flex flex-col gap-3 p-4">
            <div className="flex items-center gap-3">
              {activeFile ?
                <FileVisual file={activeFile} className={cn("shrink-0 rounded-sm", (activeFile.previewAspectRatio ?? 0.78) > 1.2 ? "w-16" : "w-9")} previewAspectRatio={0.78} renderFilePreview={renderFilePreview} />
              : <FileSystemFolderGlyph name={activeEntry.name} className="h-8 w-auto shrink-0" />}
