...
interface DockContextType {
  ...
  activeIndex: number | null // Single active index for accordion behavior
  setActiveIndex: (index: number | null) => void // Function to set active index
  animatingIndexes: number[] // Array of animating indexes (for smooth transitions)
  setAnimatingIndexes: (indexes: number[]) => void // Function to set animating indexes
  isMobile: boolean // Whether the screen is mobile-sized
}
...
function Dock({ className, children }: DockProps) {
  ...
  const [activeIndex, setActiveIndex] = useState<number | null>(null) // Single active index for accordion behavior
  ...
  useWindowResize(() => {
    ...
    setIsMobile(window.innerWidth < 640) // Update mobile state based on screen width
  })
  ...
  return (
    ...
    <DockContext.Provider
      value={{
        ...
        activeIndex, // Current active index for accordion behavior
        setActiveIndex, // Method to set active index
        animatingIndexes, // Current animating indexes
        setAnimatingIndexes, // Method to set animating indexes
        isMobile, // Whether the screen is mobile-sized
      }}
    >
      <motion.div
        ...
        className={cn(
          "absolute bottom-2 sm:bottom-4 left-1/2 -translate-x-1/2 flex items-end h-12 sm:h-14 p-1.5 sm:p-2 gap-2 sm:gap-3 rounded-xl sm:rounded-2xl",
...
function DockCard({ children, id }: DockCardProps) {
  ...
  const { isMobile } = dock // Get mobile state from dock context
  ...
  // Handle click event for accordion behavior
  const handleClick = () => {
    const cardIndex = parseInt(id)

    // Accordion behavior: only one card can be active at a time
    if (dock.activeIndex === cardIndex) {
      // Clicking the same card: close it (toggle off)
      dock.setActiveIndex(null)
      // Stop animation and reset state
      if (isAnimating.current) {
        isAnimating.current = false
        dock.setAnimatingIndexes(
          dock.animatingIndexes.filter((index) => index !== cardIndex)
        )
        opacity.set(0)
        controls.start({
          y: 0,
      ...
      // Clicking different card: open it (closes previous automatically via activeIndex change)
      dock.setActiveIndex(cardIndex)
      // Start animation for this card
      if (!isAnimating.current) {
        isAnimating.current = true
        dock.setAnimatingIndexes([...dock.animatingIndexes, cardIndex])
        opacity.set(0.5)
        controls.start({
          y: isMobile ? -20 : -24,
          transition: {
  ...
  // Effect to stop animation when this card is no longer active (accordion behavior)
  useEffect(() => {
    const cardIndex = parseInt(id)
    if (dock.activeIndex !== cardIndex && isAnimating.current) {
      // This card is no longer active, stop its animation
      isAnimating.current = false
      dock.setAnimatingIndexes(
        dock.animatingIndexes.filter((index) => index !== cardIndex)
      )
  ...
  }, [dock.activeIndex, id, dock, opacity, controls])
  ...
  const minWidth = isMobile ? 32 : 40
  const maxWidth = isMobile ? 64 : 80
  let widthSync = useTransform(distance, [-150, 0, 150], [minWidth, maxWidth, minWidth])
        ...
        {dock.animatingIndexes.includes(parseInt(id)) ? (
          ...
          >
            <motion.div
              ...
              className="w-1 sm:w-1.5 h-1 sm:h-1.5 rounded-full dark:bg-white bg-black"
              style={{ opacity }} // Bind opacity to the animated opacity spring
            />
          </motion.div>
        ) : null}
...
function DockDivider() {
  return (
    <motion.div
      className="h-full flex items-center p-1 sm:p-1.5 cursor-ns-resize"
      drag="y"
